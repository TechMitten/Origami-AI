...
async function createServer() {
  ...
  app.post('/api/llm/chat', async (req: Request, res: Response) => {
    try {
      const { baseUrl, model, messages, temperature, maxTokens, apiKey } = req.body || {};
      ...
      const effectiveKey = clientKey || getServerApiKey();
      if (!effectiveKey) return res.status(500).json({ error: 'No API key available: pass apiKey in the request body or configure LLM_API_KEY on the server' });
